 ...
 * @since 3.3
 */
public class KendallsCorrelation2 {
     ...
     */
    /*public KendallsCorrelation2(double[][] data) {
     ...
     */
    /*public KendallsCorrelation2(RealMatrix matrix) {
   ...
   /* public RealMatrix computeCorrelationMatrix(final RealMatrix matrix) {
     ...
     */
    public double correlation(final List<Node> xList, final List<Node> yList, final int topK)
            throws DimensionMismatchException {

        //final int n = xList.size();
        final long numPairs = sum(topK - 1);
        

        @SuppressWarnings("unchecked")
        Pair<Double, Double>[] pairs = new Pair[topK];
        for (int i = 0; i < topK; i++) {
            pairs[i] = new Pair<Double, Double>(xList.get(i).rank, yList.get(i).rank);
        }
        ...
        int swaps = 0;
        @SuppressWarnings("unchecked")
        Pair<Double, Double>[] pairsDestination = new Pair[topK];
        for (int segmentSize = 1; segmentSize < topK; segmentSize <<= 1) {
            for (int offset = 0; offset < topK; offset += 2 * segmentSize) {
                int i = offset;
                final int iEnd = FastMath.min(i + segmentSize, topK);
                int j = iEnd;
                final int jEnd = FastMath.min(j + segmentSize, topK);
                ...
                //logger.log(Level.INFO, "swaps += iEnd - i: " + swaps + " iEnd: " + iEnd + " i: " + i);
            }            
        ...
        tiedYPairs += sum(consecutiveYTies - 1);
        logger.log(Level.INFO, "tiedYPairs: " + tiedYPairs);
        logger.log(Level.INFO, "tiedXPairs: " + tiedXPairs);
        logger.log(Level.INFO, "tiedXYPairs: " + tiedXYPairs);
        logger.log(Level.INFO, "numPairs: " + numPairs);
        logger.log(Level.INFO, "discordant pairs: " + swaps);
        final long concordantMinusDiscordant = numPairs - tiedXPairs - tiedYPairs + tiedXYPairs - 2 * swaps;
        logger.log(Level.INFO, "concordantMinusDiscordant: " + concordantMinusDiscordant);
        //final long concordantMinusDiscordant = numPairs - tiedXPairs - tiedYPairs - 2 * swaps;
        final double nonTiedPairsMultiplied = (numPairs - tiedXPairs) * (double) (numPairs - tiedYPairs);
        logger.log(Level.INFO, "nonTiedPairsMultiplied: " + nonTiedPairsMultiplied);
